...
// Algoritmo SHA-256 em JavaScript puro para fallback (útil em conexões locais sem HTTPS/IP)
 ...
 * Se crypto.subtle não estiver disponível (HTTP/Local IP), cai para JavaScript puro de forma idêntica.
 */
export async function hashPassword(password: string): Promise<string> {
  if (typeof crypto === 'undefined' || !crypto.subtle) {
    console.warn('Web Crypto API não disponível nesta origem. Usando fallback pure-JS.');
